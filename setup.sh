#!/bin/bash
# WINTUNELING VPN - FINAL ULTIMATE V16
# Fixed by Gemini
# 1. Syntax Error in renew_user fixed
# 2. <pre> tags replaced with <code> for cleaner notification
...
REPO_URL="REPLACE_THIS_WITH_YOUR_REPO_URL"
...
function backup_menu() {
    ...
    echo -e "${CYAN}└──────────────────────────────────────────────┘${NC}"
    echo -e " [1] Backup Data (To Telegram - FILE)"
    echo -e " [2] Restore Data (From Link - DIRECT)"
    echo -e " [0] Exit"
    echo -e ""
    read -p " Select: " opt
    if [ "$opt" == "1" ]; then
        ...
        echo -e "\n${YELLOW}Sending Backup File...${NC}"
        /usr/local/bin/backup-tg
        read -n 1 -s -r -p "Press Enter to continue..."
    elif [ "$opt" == "2" ]; then
        echo -e "\n${YELLOW}[ RESTORE DATA FROM DIRECT LINK ]${NC}"
...
<i>Backup Database, Config & Domain</i>"
    ...
    # SENDING DOCUMENT (CURL -F document=@file)
    ...
    rm -rf /root/backup
    rm $FILENAME
    echo -e "Backup File sent to Telegram."
else
