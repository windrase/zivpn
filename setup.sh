#!/bin/bash
# WINTUNELING VPN - FINAL ULTIMATE V10 (Fix Loop & Backup)
# Features: Anti-Loop Landing Page, Fix Backup Telegram, Smart Update
...
function send_log() {
    ...
    curl -s -X POST "https://api.telegram.org/bot${TG_TOKEN}/sendMessage" \
        -d chat_id="${TG_ID}" -d parse_mode="html" --data-urlencode text="$1" > /dev/null 2>&1
}
...
function update_script() {
    ...
    wget -qO /root/update.sh "REPLACE_THIS_WITH_YOUR_UPDATE_URL" # Will be sed-ed
    if [ $? -eq 0 ]; then
...
function backup_menu() {
    ...
    if [ "$opt" == "1" ]; then
        ...
        /usr/local/bin/backup-tg
        echo -e "\n${GREEN}Check your Telegram!${NC}"
        read -n 1 -s -r -p "Press Enter to continue..."
    elif [ "$opt" == "2" ]; then
        echo -e "\n${YELLOW}[ HOW TO GET DIRECT LINK ]${NC}"
        echo -e "1. Forward file Backup dari Channel ke: ${CYAN}@potato_directlinkBot${NC}"
...
sed -i "s|REPLACE_THIS_WITH_YOUR_UPDATE_URL|${REPO_URL/setup.sh/update.sh}|g" $MENU_BIN
chmod +x $MENU_BIN

    # --- 4. BACKUP SCRIPT (FIXED) ---
cat << 'EOF' > $BACKUP_BIN
#!/bin/bash
...
━━━━━━━━━━━━━━━━━━━━"
    ...
    # Send using curl -F (without silent failure)
    curl -s -F chat_id="$TG_ID" -F document=@"$FILENAME" -F parse_mode="html" -F caption="$CAPTION" "https://api.telegram.org/bot$TG_TOKEN/sendDocument" > /dev/null
...
EOF
    ...
    # --- CLEAN OLD BASHRC (FIX LOOP) ---
    sed -i '/landing-page/d' /root/.bashrc
    
    # --- LANDING PAGE (FIXED TO BASHRC LOGIN ONLY) ---
    # Only runs if it is a login shell (preventing loops inside scripts)
    cat >> /root/.bashrc << 'EOF'
if [ -n "$SSH_TTY" ]; then
